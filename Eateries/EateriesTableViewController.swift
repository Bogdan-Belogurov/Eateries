...
class EateriesTableViewController: UITableViewController, NSFetchedResultsControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        searchController = UISearchController(searchResultsController: nil) // на главном экране поэтому nil
        searchController.searchResultsUpdater = self// нужно быть подписанным под протокол
        searchController.dimsBackgroundDuringPresentation = false // без затемнения
        tableView.tableHeaderView = searchController.searchBar // присвоили серчч бар нашего контроллера
        ...
        definesPresentationContext = true //не переходит на следующий экран
        ...
        let sortDescriptor = NSSortDescriptor(key: "name", ascending: true) // фильтр (как мы хотим видеть выходные данные)
        fetchRequest.sortDescriptors = [sortDescriptor] // применяем этот дескриптор
        
        if let context = (UIApplication.shared.delegate as? AppDelegate)?.coreDataStack.persistentContainer.viewContext { // пробуем добраться до контекста
            fetchResultsController = NSFetchedResultsController(fetchRequest: fetchRequest, managedObjectContext: context, sectionNameKeyPath: nil, cacheName: nil) // инициализируем контроллер и передаем два аргумента
            fetchResultsController.delegate = self
            // пытаемся получить данные
            do {
                try fetchResultsController.performFetch()
                restaurants = fetchResultsController.fetchedObjects! // если все упешно, помещаем в массив
            } catch let error as NSError {
                print(error.localizedDescription) // если нет, то ошибка
            }
        }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //        //1
        //        let call = UIAlertAction(title: "Позвонить: +7 (916) 833-23-0\(indexPath.row)", style: .default) {
